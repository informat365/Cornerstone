...
class Bar {
    ...
    draw_label() {
        var classes="bar-label ";
        if(this.task.finish){
            classes+=" bar-label-finish"
        }
        createSVG('text', {
...
class FGantt {
    constructor(wrapper, tasks, options) {
        ...
        if(options.readonly==false||options.readonly==null){
            this.bind_events();
        }
    }
    ...
    remeberScrollPos(){
        const parent_element = this.$svg.parentElement;
        if (!parent_element) return;
        this.parentScrollPos=parent_element.scrollLeft;
    }
    //
    resetScrollPos(){
        const parent_element = this.$svg.parentElement;
        if (!parent_element) return;
        parent_element.scrollLeft=this.parentScrollPos;
    }
    //
    bind_grid_click() {
        $.on(
