         ...
         <BizTable :fixed="true" @change="loadData" :page="pageQuery" :value="tableData" >
            ...
            <template slot="tbody">
                <tr v-for="item in tableData" :key="item.id" class="table-row">
                    ...
                    <td style="max-width:250px">{{item.cron}}</td>
...
export default {
    ...
    methods:{
        pageLoad(){
            this.loadPageStatus("CmdbRobotPage")
            this.loadData();
        },
        pageMessage(type){
            if(type=='cmdrobot.edit'){
                this.loadData(true);
            }
        },
        loadData(resetPage){
            if(resetPage){
             ...
             this.savePageStatus("CmdbRobotPage")
        ...
        settingItem(item){
            app.showDialog(CmdbRobotEditDialog,{
                id:item.id
            })
        },
        showCreateDialog(){
