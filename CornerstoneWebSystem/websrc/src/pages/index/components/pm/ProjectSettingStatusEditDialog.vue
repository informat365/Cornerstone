 ...
 <i18n>
{
    "en": {
        ...
        "备注":"Remark",
        "类型":"Type",
        "开始状态只能有一个":"There can only be one start state, which is given by default when an object is created.",
        "结束状态表示对象流转已完成":"The end state indicates that the object flow has been completed and the object in the end state can no longer be edited.",
        "颜色":"Color",
        "只允许以下成员操作":"Only the following member operations are allowed",
        "当变更为此状态时将负责人设置为":"Change owner to ",
    ...
    "zh_CN": {
        ...
        "类型":"类型",
        "只允许以下成员操作":"只允许以下成员操作",
        "开始状态只能有一个":"开始状态只能有一个，对象创建时会默认赋予这个状态",
        "结束状态表示对象流转已完成":"结束状态表示对象流转已完成，结束状态的对象不能再进行编辑",
        "颜色":"颜色",
        "当变更为此状态时将负责人设置为":"当变更为此状态时将负责人设置为",
        "检查字段已经设置":"检查字段已经设置",
        "删除此状态":"删除此状态",
        "继续创建下一个":"继续创建下一个",
        "创建":"创建",
    ...
    <Form @submit.native.prevent  ref="form" :rules="formRule" :model="formItem" label-position="top" style="height:500px;padding:25px">
        <Row>
         ...
         <Row>
            <Col span="11">
                <FormItem :label="$t('类型')">
            <DataDictRadio  :disabled="disabled" v-model="formItem.type" type="ProjectStatusDefine.type" />
            <div v-if="formItem.type==1" class="desc-info">{{$t('开始状态只能有一个')}}</div>
            <div v-if="formItem.type==3" class="desc-info">{{$t('结束状态表示对象流转已完成')}}</div>
        </FormItem>
            </Col>
            <Col offset="2" span="11">
        ...
        <FormItem :label="$t('只允许以下成员操作')">
            <Select :disabled="disabled" transfer v-if="changeOwnerListLoaded" v-model="formItem.permissionOwnerList" multiple clearable style="width:100%">
        ...
        <FormItem :label="$t('检查字段已经设置')">
             <Select :disabled="disabled" transfer v-model="formItem.checkFieldList" multiple clearable style="width:100%">
                <Option v-if="item.isShow" v-for="item in fieldList"  :value="item.id" :key="'f'+item.id">{{ item.name }}</Option>
            </Select>
        </FormItem>
...
<script>
    export default {
        ...
        methods: {
            pageLoad(){
                if(this.args.item){
                    this.formItem=JSON.parse(JSON.stringify(this.args.item));
                }else{
                ...
                this.loadRoleList();
                this.loadFieldList();
                this.disabled = this.args.project&&this.args.project.isFinish&&this.args.objectType!=0;
            ...
            loadFieldList(){
                app.invoke('BizAction.getProjectFieldDefineInfoList',[app.token,
                    this.formItem.projectId,this.formItem.objectType],(list)=>{
                        this.fieldList=list;
                });
            },
            deleteField(){
                app.confirm(this.$t('确定要删除状态',[this.formItem.name]),()=>{
                     app.invoke('BizAction.deleteProjectStatusDefine',[app.token,this.formItem.id],(info)=>{
            ...
            confirmForm(){
                var action=this.formItem.id==0?'BizAction.addProjectStatusDefine':"BizAction.updateProjectStatusDefine";
