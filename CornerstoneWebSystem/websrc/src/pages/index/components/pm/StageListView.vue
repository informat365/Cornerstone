        ...
        <Row>
            ...
            <Col span="12">
                <div class="table-info">
                    <span class="table-count">{{$t('个阶段',[list.length])}}</span>
                </div>
            </Col>
            <Col span="6" class="right-opt">
                ...
                <IconButton
                    v-if="prjPerm('stage_edit')&&!projectDisabled"
                    icon="md-add"
                    ...
                    <Card
                        ...
                        <Row>
                            ...
                            <Col span="12" class="text-right">
                                ...
                                <IconButton style="padding-left: 0px;" v-if="prjPerm('stage_edit')"
                        ...
                        <Row>
                            <Col span="24">
                                ...
                                <span
                                    ...
                                    <span v-if="getLeftDays(item.endDate)==0"
                                          style="font-weight:bold">{{$t('今天到期')}}</span>
                                    <span
                                        v-if="getLeftDays(item.endDate)<0"
                                        style="font-weight:bold;color:#FF355A"
                                    >{{$t('超期',[getLeftDays(item.endDate)*-1])}}</span>
                                    <span style="font-weight:bold;margin-left: 5px;">{{$t('总工期',[item.workDay])}}</span>
                                </template>
                            </Col>
                        </Row>
                    </Card>
            ...
            <Table
                ...
                <template slot="remark" slot-scope="{ row }">
                    <div v-if="row.status!=3">
                        ...
                        <span style="font-weight:bold;margin-left: 5px;">{{$t('总工期',[row.workDay])}}</span>
                    </div>
                </template>
                <template slot="opt" slot-scope="{ row }">
                    <IconButton v-if="prjPerm('stage_edit')" @click="showEditDialog(row)"
                                icon="ios-settings-outline"></IconButton>
                </template>
            </Table>
...
<script>
    ...
    import StageViewRelTaskRow from "./StageViewRelTaskRow";
    ...
    export default {
                ...
                columns: [
                    {
                        ...
                        width: 30,
                        render: (h, params) => {
                            return h(StageViewRelTaskRow, {
                                props: {
                                    datas: params.row.children
                                }
                            })
                        }
                    },
        ...
        mounted() {
            var t = app.loadObject("Stage.viewType");
            if (t != null) {
                this.viewType = t;
            }
            this.loadData();
        ...
        methods: {
            ...
            loadAssociateList(query) {
                app.invoke(
                    "BizAction.getStageAssociateList",
                    [app.token, query],
                    info => {
                        if (!!info && !Array.isEmpty(info)) {
                            let map = new Map();
                            info.forEach(a => {
                                var associates = map.get(a.stageId);
                                if (!associates) {
                                    map.set(a.stageId, [a])
                                } else {
                                    associates.push(a);
                                    map.set(a.stageId, associates)
                                }
                            });
                            for (let i = 0; i < this.list.length; i++) {
                                var stageId = this.list[i].id;
                                var associates = map.get(stageId);
                                if (!!associates) {
                                    associates.sort((a, b) => {
                                        return -(a.type - b.type);
                                    })
                                    this.$set(this.list[i], 'children', associates);
                                }
                            }
                        }
            ...
            getUsedDays(stage) {
                var now = new Date();
                if (stage.startDate.getTime() < now.getTime()) {
                    return 0;
                } else {
                    if (stage.endDate.getTime() >= now.getTime()) {
                        return window.dateDiff(stage.startDate, new Date());
                    }
                }
                return window.dateDiff(stage.startDate, new Date());
            },
            changeViewType(t) {
                this.viewType = t;
                app.saveObject("Stage.viewType", t);
            },
            showEditDialog(item) {
            ...
            showDetail(item) {
                app.showDialog(StageInfoDialog, {
                    id: item.id
                });
            }
        }
