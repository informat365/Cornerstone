    ...
    <Modal class="drawer-detail"
        ...
        <div class="drawer-detail-header" slot="header">
            <div class="drawer-detail-header-title">
                {{!hasPermissionUpdate?'查看':'编辑'}}系统版本
            </div>
        ...
        <template v-if="detail">
            <div style="padding: 16px">
                <Form class="drawer-version-form" label-position="left" :label-width="100">
                    <Row :gutter="20">
                        <Col :span="24">
                            <FormItem class="name">
                                ...
                                <template v-else>
                                    <Input :disabled="!hasPermissionUpdate"
                        ...
                        <Col :span="12">
                            <FormItem label="版本号" prop="versionNo">
                                <Input :disabled="!hasPermissionUpdate"
                                    :maxlength="20" v-model.trim="detail.versionNo"
                                    @on-blur="onUpdateVersion('versionNo')"/>
                            </FormItem>
                        </Col>
                        <Col :span="12">
                            <FormItem
                                ...
                                <CompanyUserSelect
                                    ...
                                    v-model="detail.ownerAccountIdList"
                                    :default-list="detail.ownerAccountList"
                                    @on-change="onUpdateVersion('ownerAccountIdList')"
                                    clearable
                                    multiple
                                    placeholder="版本负责人"
                                    style="width: 300px;"
...
<script>
    ...
    import VersionStatusSelect from '../VersionRepository/components/VersionStatusSelect';
    import VersionTaskList from '../VersionRepository/components/VersionTaskList';

    export default {
        name: 'DashboardVersionEditDialog',
        ...
        data() {
            return {
                ...
                versionId: 0,
                ownerAccount: [],
                originVersionName: null,
                repeatVersionName: null,
                readonly:false
            }
        ...
        methods: {
            ...
            loadData() {
                this.loading = true;
                app.invoke('BizAction.getCompanyVersionById', [app.token, this.versionId], (res) => {
                    ...
                    this.originVersionName = res.name;
                    this.detail = {
                        ...res,
                    ...
                    // this.ownerAccount = [{
                    //     id: res.ownerAccountId,
                    //     name: res.ownerAccountName,
                    // }]
                    this.loading = false;
            ...
            onUpdateVersion(field) {
                ...
                this.loading = true;
                app.invoke('BizAction.updateCompanyVersion', [app.token, {...this.detail}], () => {
                    app.toast('更新成功');
            ...
            onDeleteVersion() {
                this.twiceFactorValid(() => {
                    this.loading = true;
                    app.invoke('BizAction.deleteCompanyVersion', [app.token, this.detail.id], () => {
                        app.toast('删除版本成功');
                        app.postMessage('version.item.edit');
                        this.showModal = false;
                        this.loading = false;
            ...
            twiceFactorValid(callback) {
                ...
                _this.$Modal.confirm({
                    render: h => {
                        return h("Input", {
                            props: {
                                value: _this.repeatVersionName,
                                autoFocus: true,
                                placeholder: "请输入版本名称以进行确认删除"
                            },
                            on: {
                                input: val => {
                                    _this.repeatVersionName = val;
                                }
                            }
                        });
                    },
                    onOk: () => {
                        if (_this.repeatVersionName) {
                            if (_this.repeatVersionName !== this.originVersionName) {
                                app.toast("输入版本名称不匹配");
                                _this.repeatVersionName = null;
                            } else {
                                if (callback) {
                                    callback.apply();
                                }
