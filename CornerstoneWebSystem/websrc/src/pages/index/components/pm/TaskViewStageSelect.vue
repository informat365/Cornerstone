...
<template>
    <Poptip ref="poptip" transfer :disabled="!canedit" class="poptip-full" v-model="visible"  >
        <div class="stage-label">
            <div v-if="searchable"  class="it-name text-no-wrap"  >
                【<template v-if="selectedItem">{{selectedItem.name}}</template>】
            </div>
            <div v-else class="it-name text-no-wrap">
                <template v-if="selectedItem">{{selectedItem.name}}</template>
                <template v-else>未设置</template>
            </div>
            <Icon v-if="canedit" class="stage-label-icon" size="16" type="ios-arrow-down" />
        </div>
        <div slot="content" >
            <div v-if="searchable" style="padding:10px;padding-bottom:0">
                <Input v-model="searchText" :placeholder="$t('搜索阶段')" icon="ios-search"></Input>
            </div>
            ...
            <div  class="popup-box scrollbox">
                <Row @click.native="selectItem(item)" class="popup-item-name"
                    ...
                    <Col span="21" class="name-row">
                        <div class="item-label list-it-name text-no-wrap" :class="'it-status-'+item.status" v-tooltip="item.name" v-text-overflow="item.name"></div>
...
export default {
    name:"TaskViewStageSelect",
    props: ['list','value','editable','searchable'],
    directives: {
        TextOverflow,
        Tooltip,
    },
    ...
    computed:{
        filteredList(){
            var t=[];
            if(!this.searchable){
                this.stageList.unshift({id:0,name:'无'})
            }
            if(this.searchText==null||this.searchText==""){
                return this.stageList;
            }
            for(var i=0;i<this.stageList.length;i++){
                var q=this.stageList[i];
