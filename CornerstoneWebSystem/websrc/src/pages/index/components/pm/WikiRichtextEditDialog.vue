<style lang="less" scoped>
    .wiki-richtext-edit-dialog {
        /deep/ .ivu-modal-body {
            height: calc(100vh - 51px) !important;
            overflow: hidden;
        }
    }

    .page {
        position: relative;
        height: 100%;
        width: 100%;
        overflow: auto;
        overflow-scrolling: touch;
        background-color: #f1f4f5;
    }

    .option-bar {
        align-items: center;
        display: flex;
        background-color: #fff;
        width: 100%;
        height: 22px;
        max-height: 22px;
    }

    .opt-right {
        text-align: right;
    }

    .table-info-bar {
        font-size: 12px;
        font-weight: bold;
        color: #999;
        text-align: right;
    }

    .doc-name {
        border-bottom: 1px solid #eee;
        padding-bottom: 10px;
    }

    .doc-name input {
        width: 100%;
        border: none;
        outline: 0;
        font-weight: bold;
        font-size: 18px;
        margin-top: 10px;
        color: #333;
        margin-left: 15px;
    }

    .mail-user-box {
        border: 1px solid #dddee1;
        padding-left: 5px;
        padding-right: 5px;
        cursor: pointer;
    }
</style>

<i18n>
    {
    "en": {
    "创建页面": "Create",
    "已保存": "saved",
    "未保存": "unsaved",
    "输入文档标题": "Input title",
    "保存": "Save",
    "保存为修订版": "Save as Version",
    "关闭": "Close",
    "复制": "Copy",
    "请输入文档标题": "Input title",
    "页面大纲": "Outline",
    "保存成功": "Success",
    "发布成功": "Success"
    },
    "zh_CN": {
    "创建页面": "创建页面",
    "已保存": "已保存",
    "未保存": "未保存",
    "输入文档标题": "输入文档标题",
    "保存": "保存",
    "保存为修订版": "保存为修订版",
    "关闭": "关闭",
    "复制": "复制",
    "请输入文档标题": "请输入文档标题",
    "页面大纲": "页面大纲",
    "保存成功": "保存成功",
    "发布成功": "发布成功"
    }
    }
</i18n>

<template>
    <Modal
        ref="dialog"
        v-model="showDialog"
        :closable="false"
        :mask-closable="false"
        :loading="false"
        :title="$t('创建页面')"
        class="wiki-richtext-edit-dialog fullscreen-modal"
        width="100%"
        :footer-hide="true">
        <Row class="option-bar" slot="header">
            <Col span="6" class="opt-left ">
        <span v-if="lastSaveTime>0" style="color:#999">
           {{$t('已保存')}} {{lastSaveTime|fmtDateTime}}
        </span>
                <span v-if="lastSaveTime==0" style="color:#999">
          {{$t('未保存')}}
        </span>
            </Col>
            <Col span="12" class="opt-left" style="text-align:center">
                <div
                    class="simditor simditor-mac simditor-toolbar-noborder" style="border:none"
                    ref="wikiEditorToolbar"></div>
            </Col>
            <Col span="6" class="opt-right">
                <Form inline>
                    <FormItem>
                        <Button @click="saveDoc()" type="default" icon="ios-cloud-upload">{{$t('保存')}}</Button>
                    </FormItem>
                    <FormItem>
                        <Button @click="releasePage" type="success" icon="ios-paper-plane-outline">{{$t('保存为修订版')}}
                        </Button>
                    </FormItem>
                    <FormItem>
                        <Button @click="closePage" type="default">{{$t('关闭')}}</Button>
                    </FormItem>
                </Form>
            </Col>
        </Row>
        <div
            ref="wikiEditor"
            class="page wiki-editor scrollbox">
            <div
                class="page-wrap">
                <div class="page-content simditor-toolbar-noborder" style="min-height:750px;width:750px;">
                    <div class="doc-name"><input
                        maxlength="50"
                        v-model.trim="formItem.name"
                        type="text"
                        :placeholder="$t('输入文档标题')"></input></div>
                    <textarea ref="wikiDocEditor"></textarea>
                </div>
            </div>
        </div>
        <HtmlCatalog
            ref="htmlCatalog"
            class="note-catalog"
            :offset-top="72"
            :scroll-box="$refs.wikiEditor"/>
    </Modal>
</template>

<script>

    export default {
        mixins: [componentMixin],
        data() {
            return {
                showSendMailBox: false,
                formItem: {
                    id: 0,
                    name: null,
                    content: null,
                },
                lastSaveTime: 0,
                outlineList: [],
                checkInterval: 0,
                autoSaveInterval: 0,
                isCompanyWiki: false
            };
        },
        watch: {
            showDialog(val) {
                if (val == false) {
                    this.removeListener();
                }
            },
        },
        methods: {
            pageLoad() {
                this.isCompanyWiki = !(this.args.projectId||this.args.parentId||this.args.wikiId);
                this.formItem.type = 1;
                this.formItem.wikiId = this.args.wikiId;
                this.formItem.parentId = this.args.parentId;
                if (this.args.id) {
                    app.invoke(`BizAction.get${this.isCompanyWiki?'Company':''}WikiPageById`, [app.token, this.args.id], (info) => {
                        this.formItem = info;
                        if (info.content) {
                            this.editor.setValue(info.content);
                            this.getDocOutline();
                        }
                        if (this.args.copy) {
                            this.formItem.id = 0;
                            this.formItem.originalId = null;
                            this.formItem.name = this.formItem.name + this.$t('复制');
                        }
                    });
                }
                this.createEditor();
                this.addKeyListener();
                //
                this.checkInterval = setInterval(this.getDocOutline, 2000);

            },
            removeListener() {
                if (this.checkInterval > 0) {
                    clearInterval(this.checkInterval);
                }
                document.removeEventListener('keydown', this.keyHandler);
            },
            addKeyListener() {
                document.addEventListener('keydown', this.keyHandler, false);
            },
            keyHandler(e) {
                if (e.keyCode == 83 && (navigator.platform.match('Mac') ? e.metaKey : e.ctrlKey)) {
                    e.preventDefault();
                    this.saveDoc();
                }
            },
            createEditor() {
                var tabBarFloat = true;
                var toolbar = ['title', 'bold', 'italic', 'underline', 'strikethrough', 'fontScale', 'color', 'mark',
                    '|', 'ol', 'ul', 'blockquote', 'code', 'table',
                    '|', 'link', 'image', 'hr',
                    '|', 'indent', 'outdent', 'alignment'];
                var editor = new Simditor({
                    textarea: this.$refs.wikiDocEditor,
                    toolbar: toolbar,
                    toolbarFloat: tabBarFloat,
                    pasteImage: true,
                    upload: {
                        url: app.serverAddr + '/p/file/upload?token=' + app.token,
                        fileKey: 'file',
                        connectionCount: 3,
                    },
                });
                this.editor = editor;
                editor.cleanNodeFormat = true;
                editor.on('valuechanged', () => {
                    this.formItem.content = this.editor.getValue();
                    this.$nextTick(() => {
                        this.$refs.htmlCatalog.analyzeElHtml($('.page-content .simditor-body')[0]);
                    });
                });
                //
                $('.wiki-editor .simditor-toolbar').appendTo(this.$refs.wikiEditorToolbar);
            },
            saveDoc(callback) {
                if (this.formItem.name == null || this.formItem.name == '') {
                    app.toast(this.$t('请输入文档标题'));
                    return;
                }
                var action = this.formItem.id == 0 ? `BizAction.add${this.isCompanyWiki?'Company':''}WikiPage`:`BizAction.update${this.isCompanyWiki?'Company':''}WikiPage`;
                this.formItem.content = this.editor.getValue();
                app.invoke(action, [app.token, this.formItem], (id) => {
                    if (id) {
                        this.formItem.id = id;
                    }
                    this.lastSaveTime = new Date().getTime();
                    app.toast(this.$t('保存成功'));
                    app.postMessage(this.isCompanyWiki?'company.wiki.edit':'wikipage.edit', this.formItem.id);
                    // app.postMessage('wikipage.edit', this.formItem.id);
                    if (callback != null) {
                        callback();
                    }
                });
            },
            releasePage() {
                this.saveDoc(() => {
                    app.invoke(`BizAction.release${this.isCompanyWiki?'Company':''}WikiPage`, [app.token, this.formItem.id], (id) => {
                        app.toast(this.$t('发布成功'));
                        app.postMessage(this.isCompanyWiki?'company.wiki.edit':'wikipage.edit', id);
                        this.closePage();
                    });
                });
            },
            clickOutline(item) {
                item.element.scrollIntoView();
            },
            getDocOutline() {
                var editorBody = this.$el.getElementsByClassName('simditor-body')[0];
                var children = editorBody.children;
                this.outlineList = [];
                for (var i = 0; i < children.length; i++) {
                    var t = children[i];
                    var q = t.tagName.toUpperCase();
                    if (q == 'H1' || q == 'H2' || q == 'H3') {
                        this.outlineList.push({
                            type: q,
                            name: t.textContent,
                            element: t,
                        });
                    }
                }
            },

            closePage() {
                this.removeListener();
                this.showDialog = false;
            },
        },
    };
</script>
