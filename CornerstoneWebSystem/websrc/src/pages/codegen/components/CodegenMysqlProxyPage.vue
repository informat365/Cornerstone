      ...
      <div style="padding:25px;background-color:#fff">
       ...
       <BizTable :fixed="true" :page="pageQuery" @change="loadData" :value="tableData">
            <template slot="thead">
                    <tr>
                     <th>{{$t('名称')}}</th>
                      <th style="width:100px">{{$t('代理端口')}}</th>
                       <th style="width:150px">{{$t('数据库用户名')}}</th>
                      <th style="width:200px">{{$t('主机')}}</th>
                      <th style="width:60px">{{$t('端口')}}</th>
                      <th style="width:150px">{{$t('创建人')}}</th>
                      <th style="width:150px;text-align:right">{{$t('操作')}}</th>     
                    </tr>
            </template>
            <template slot="tbody">
...
export default {
    ...
    methods:{
        ...
        loadData(resetPage){
            ...
            app.invoke('DesignerAction.getDesignerMysqlProxyInstanceList',[app.token,query],(info)=>{
                this.tableData=info.list;
                this.pageQuery.total=info.count;
            })   
        },
        showDbChange(item){
            app.loadPage('codegen_database_change?id='+item.id)
        },
        showCreateDialog(){
            app.showDialog(CodegenMysqlProxyEditDialog)
        },
        showEditDialog(item){
             app.showDialog(CodegenMysqlProxyEditDialog,{
                 id:item.id
        ...
        deleteItem(item){
            app.confirm(this.$t('确认要删除此数据库代理吗'),()=>{
                    app.invoke('DesignerAction.deleteDesignerMysqlProxyInstance',[app.token,item.id],function(obj){
