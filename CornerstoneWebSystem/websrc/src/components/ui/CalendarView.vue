...
export default {
	...
	methods: {
		...
		*/
		...
		getEventTitle(e) {
			if (!this.showEventTimes) {
				if(e.originalEvent.type=='提醒'){
					return  "<span class='remind-item'>"+
						'<i style="font-size:14px" class="ivu-icon ivu-icon-ios-alarm"></i>'+
						e.originalEvent.type+" </span>"+e.title
				}else{
					return  "<span class='task-item' style='border-left: 4px solid "
					+e.originalEvent.priorityColor+"'>"+e.originalEvent.type+" </span>"+e.title
				}
			}
...
.remind-item{
	color:#fff;
	background-color: rgb(16, 181, 247) !important;
	padding-left:5px;
